...
fn get_next(n: usize, pos: usize, idx: usize, xyz: &Vec<(usize, usize, usize)>) -> usize {
    let mut state = vec![0; 15];
    ...
    state[y-1] = 1 - state[y-1];
    state[z-1] = 1 - state[z-1];
...
fn main() {
    ...
    let mut g = vec![vec![0; 0]; 1<<n];
    let mut dist = vec![-1; 1<<n];

    for i in 0..(1<<n) {
        for j in 0..m {
            let next_state = get_next(n, i, j, &xyz);
            g[i].push(next_state);
        }
    }
    let goal = (1 << n) - 1;
