...
fn main() {
    ...
    let mut higher_points = Vec::new();
    
    for i in 0..m {
        higher_points.push((a[i], i));
    }
    higher_points.sort();
    higher_points.reverse();
    
    for player in 0..n {
        let mut target = 0;
        for opponent in 0..n {
            if player == opponent {continue;}
            chmax!(target, players[opponent]);
        }
        let mut current_point = players[player];
        let mut count = 0;
        for (point, index) in &higher_points {
            if current_point >= target {
                break;
            }
            if s[player][*index] == 'o' {continue;}
