...
fn func(v: &Vec<usize>, budget: usize, limit: usize) -> bool {
    let mut cost = 0;
...
#[allow(dead_code)]
fn lower_bound(arr: &Vec<usize>, budget: usize) -> usize {
    let mut ok = 0;
    let mut ng = usize::MAX;

    while ng - ok > 1 {
        let mid = ok + (ng - ok) / 2;
        if func(&arr, budget, mid) {
            ok = mid;
        } else {
            ng = mid;
        }
