...
fn main() {
    input! {
        n: usize,
        a: [(isize, isize, isize); n],
    }

    for cx in 0..=100 {
        for cy in 0..=100 {
            let mut can = true;
            let mut estimated_height = -1;
            for (nx, ny, nh) in a.clone() {
                if nh != 0 {
                    estimated_height = nh + (cx - nx).abs() + (cy - ny).abs();
                }
            }
            for (nx, ny, nh) in a.clone() {
                let tmp_h = max(estimated_height - (nx - cx).abs() - (ny - cy).abs(), 0);
                if tmp_h != nh {
                    can = false;
                }
            }
            ...
            if can {
                println!("{} {} {}", cx, cy ,estimated_height);
                return;
            }
        }
    }
