...
fn is_valid(coordinates: (isize, isize), s: &Vec<Vec<char>>) -> bool {
    if coordinates.0 < 0 || coordinates.1 < 0 || coordinates.0 >= 9 || coordinates.1 >= 9 {
        return false;
    }

    if s[coordinates.0 as usize][coordinates.1 as usize] != '#' {
        return false;
    }
    true
}
...
fn main() {
    ...
    for i in 0..9 as isize {
        for j in 0..9 as isize {
            if s[i as usize][j as usize] != '#' { continue; }
            ...
            for dy in -9..=9 {
                for dx in -9..=9 {
                    ...
                    if is_valid((i, j), &s) && is_valid(b, &s) && is_valid(c, &s) && is_valid(d, &s) {
                        let mut v = [
                            (i, j),
                            b,
                            c,
