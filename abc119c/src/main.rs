...
fn dfs(
    ...
    if a_count+b_count+c_count+n_count == n as isize {
        if a_count == 0 || b_count == 0 || c_count == 0 {
            return;
        }
        let a_cost = max(0, a_count-1) * 10 + (a-a_len).abs();
        let b_cost = max(0, b_count-1) * 10 + (b-b_len).abs();
        let c_cost = max(0, c_count-1) * 10 + (c-c_len).abs();
        chmin!(*ans, a_cost+b_cost+c_cost);
        return;
    }

    // A
    dfs(n, a, b, c, l, ans, depth+1, a_len+l[depth], b_len, c_len, a_count+1, b_count, c_count, n_count);

    // B
    dfs(n, a, b, c, l, ans, depth+1, a_len, b_len+l[depth], c_len, a_count, b_count+1, c_count, n_count);

    // C
    dfs(n, a, b, c, l, ans, depth+1, a_len, b_len, c_len+l[depth], a_count, b_count, c_count+1, n_count);

    // No
    dfs(n, a, b, c, l, ans, depth+1, a_len, b_len, c_len, a_count, b_count, c_count, n_count+1);
...
fn main() {
    ...
    dfs(n, a, b, c, &l, &mut ans, 0, 0, 0, 0, 0, 0, 0, 0);
