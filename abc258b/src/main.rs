...
fn main() {
    ...
    for dy in -1..=1 {
        for dx in -1..=1 {
            ...
            for start_y in 0..n as isize {
                for start_x in 0..n as isize {
                    let mut cy = start_y;
                    let mut cx = start_x;

                    let mut tmp = (a[start_y as usize][start_x as usize].to_digit(10).unwrap()) as usize;
                    
                    for _ in 0..n-1 {
                        cy = (cy + dy).rem_euclid(n as isize);
                        cx = (cx + dx).rem_euclid(n as isize);
                        tmp = tmp * 10 + (a[cy as usize][cx as usize].to_digit(10).unwrap()) as usize;
                    }
