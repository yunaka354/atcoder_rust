...
fn main() {
    ...
    for i in 0..H {
        for j in 0..W {
            let entry = map.entry(a[i][j]).or_insert(0);
            *entry += 1;
        }
    }
    ...
    for i in 0..(H - h + 1) {
        for j in 0..(W - w + 1) {
            if j == 0 {
                ...
                for y in 0..h {
                    for x in 0..w {
                        let entry = copied.entry(a[i + y][j + x]).or_insert(0);
                        *entry -= 1;
                        if *entry == 0 {
                            copied.remove(&a[i + y][j + x]);
                        }
                    }
                }
            } else {
                for y in 0..h {
                    // 元に戻す
                    let entry = copied.entry(a[i + y][j - 1]).or_insert(0);
                    *entry += 1;

                    // 削る
                    let entry = copied.entry(a[i + y][j + w - 1]).or_insert(0);
                    *entry -= 1;
                    if *entry == 0 {
                        copied.remove(&a[i + y][j + w - 1]);
                    }
                }
            }
