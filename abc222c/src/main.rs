...
fn main() {
    ...
    for i in 0..2*n {
        players.push((0, i)); // (win, player_id)
    }
    
    let janken = |p1: char, p2: char| {
        // 0: win, 1: draw, 2: lose
        if p1 == p2 {
            return 1;
        }
    ...
    for round in 0..m {
        for pair in 0..n {
            let p1 = players[pair*2];
            let p2 = players[pair*2+1];
            let result = janken(a[p1.1][round], a[p2.1][round]);
            if result == 0 {
                players[pair*2].0 += 1;
            } else if result == 2 {
                players[pair*2+1].0 += 1;
            }
        }
        players.sort_by(|a, b| b.0.cmp(&a.0).then(a.1.cmp(&b.1)));
    }
