...
fn main() {
    ...
    let mut plans = vec![vec![0 as usize; 3]; n];
    let mut dp = vec![vec![0 as usize; 3]; n];
    ...
    for i in 1..n {
        for j in 0..3 {
            if j == 0 {
                chmax!(dp[i][j], dp[i-1][1] + plans[i][j]);
                chmax!(dp[i][j], dp[i-1][2] + plans[i][j]);
            } else if j == 1 {
                chmax!(dp[i][j], dp[i-1][0] + plans[i][j]);
                chmax!(dp[i][j], dp[i-1][2] + plans[i][j]);
            } else {
                chmax!(dp[i][j], dp[i-1][0] + plans[i][j]);                
                chmax!(dp[i][j], dp[i-1][1] + plans[i][j]);
    ...
    println!("{}", max!(dp[n-1][0], dp[n-1][1], dp[n-1][2]));
}