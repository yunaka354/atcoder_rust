...
fn main() {
    ...
    let mut graph = vec![vec![(0, 0);0]; n];

    for (a,b,c,d) in abcd {
        if d == 1 {
            graph[a-1].push((b-1, 10000*c-1));
            graph[b-1].push((a-1, 10000*c-1));
        } else {
            graph[a-1].push((b-1, 10000*c));
            graph[b-1].push((a-1, 10000*c));
        }
    }

    let mut fixed = vec![false; n];
    let mut que = BinaryHeap::new();
    let mut cost = vec![MAX; n];
    cost[0] = 0;
    que.push(Reverse((0,0)));

    while let Some(Reverse((_current_cost, position))) = que.pop() {
        if fixed[position] {continue;}

        fixed[position] = true;
        for (next, edge_cost) in &graph[position] {
            if cost[*next] > cost[position] + edge_cost {
                cost[*next] = cost[position] + edge_cost;
                que.push(Reverse((cost[*next], *next)));
            }
        }
    }

    let distance = (cost[n-1] + 9999) / 10000;
    let trees = distance * 10000 - cost[n-1];
