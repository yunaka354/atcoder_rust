...
fn push(
    que: &mut VecDeque<Vec<Vec<usize>>>,
    dist: &mut HashMap<Vec<Vec<usize>>, usize>,
    s: &Vec<Vec<usize>>,
    d: usize,
) {
    if dist.get(s).is_none() {
        dist.insert(s.clone(), d);
