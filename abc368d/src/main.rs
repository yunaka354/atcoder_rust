...
fn main() {
    ...
    let mut investigate = VecDeque::new();

    for node in 0..n {
        if edges[node].len() <= 1 && !v.contains(&node) {
            investigate.push_back(node);
        }
    }

    while let Some(node) = investigate.pop_front() {
        ans -= 1;
        let tos: Vec<usize> = edges[node].iter().cloned().collect();
        for to in tos {
            edges[to].remove(&node);
            if edges[to].len() <= 1 && !v.contains(&to) {
                investigate.push_back(to);
            }
        }
    }
