...
fn main() {
    ...
    let mut flag = -1; // 全体の管理フラグ
    let mut changed_index = vec![0; n]; // 変更された時のインデックス。
    let mut last_index = 0; // 最後に変更された時のインデックス。

    for i in 0..q {
        input! {t: usize, x: usize, c: char}
        if t == 1 {
            s[x - 1] = c;
            changed_index[x - 1] = i;
        } else {
