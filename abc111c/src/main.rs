...
fn main() {
    ...
    for i in (0..n).step_by(2) {
        let entry = even.entry(v[i]).or_insert(0);
        *entry += 1;
    }
    
    for i in (1..n).step_by(2) {
        let entry = odd.entry(v[i]).or_insert(0);
        *entry += 1;
    }
    
    let mut v_even: Vec<(usize, usize)> = even.iter().map(|(&k, &v)| (k, v)).collect();
    let mut v_odd: Vec<(usize, usize)> = odd.iter().map(|(&k, &v)| (k, v)).collect();
    
    v_even.sort_by(|a, b| b.1.cmp(&a.1));
    v_odd.sort_by(|a, b| b.1.cmp(&a.1));
    
    if v_even[0].0 != v_odd[0].0 {
        println!("{}", n/2-v_even[0].1 + n/2-v_odd[0].1);
        return;
    }

    println!("{}", min(n/2-v_even[0].1+n/2-v_odd[1].1, n/2-v_even[1].1+n/2-v_odd[0].1));
}