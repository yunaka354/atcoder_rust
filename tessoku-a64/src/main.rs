...
fn main() {
    ...
    let mut cur = vec![MAX; n];

    let mut heap = BinaryHeap::new();
    cur[0] = 0;
    heap.push(Reverse((cur[0], 0)));
    while let Some(Reverse(item)) = heap.pop() {
        let (current_cost, pos) = item;

        if kakutei[pos] { continue }

        kakutei[pos] = true;
        for (next, cost) in &g[pos] {
            if cur[*next] > current_cost + cost {
                cur[*next] = current_cost + cost;
