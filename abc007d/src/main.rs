...
fn solve(n: usize) -> usize {
    ...
    // dp[決めた桁数][未満フラグ][4か9を含むか]
    let mut dp = vec![vec![vec![0 as usize; 2]; 2] ;l+1];
    dp[0][0][0] = 1;

    for i in 0..l {
        let digit = n[i].to_digit(10).unwrap();
        for j in 0..2 {
            for k in 0..2 {
                let mut upto = digit; // ループ限界値
                // 未満フラグが立っていれば9まで入れていい
                if j == 1 {
                    upto = 9;
                }
                for d in 0..=upto {
                    let less_than_flag = (j > 0 || d < digit) as usize; // すでに未満になることが確定しているか、今回確定するか
                    let prohibit_number_flag = (k > 0 || d == 4 || d == 9) as usize; // すでに4か9が使われているか、今回初めて使うか
                    dp[i+1][less_than_flag][prohibit_number_flag] += dp[i][j][k];
                }
            }
        }
    }
