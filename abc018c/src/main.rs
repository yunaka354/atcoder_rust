...
fn main() {
    ...
    for i in 1..=r {
        // 左から右に見て行く
        for j in 1..=c {
            dp[i][j] = *([dp[i][j], dp[i - 1][j] + 1, dp[i][j - 1] + 1]) // 上と左と比較
                .iter()
                .min()
                .unwrap();
        }
        // 右から左に見て行く
        for j in (1..=c).rev() {
            dp[i][j] = *([dp[i][j], dp[i - 1][j] + 1, dp[i][j + 1] + 1]) // 上と右と比較
                .iter()
                .min()
                .unwrap();
        }
    ...
    for i in (1..=r).rev() {
        // 左から右に
        for j in 1..=c {
            dp[i][j] = *([dp[i][j], dp[i + 1][j] + 1, dp[i][j - 1] + 1]) // 下と左と比較
                .iter()
                .min()
                .unwrap();
        }
        // 右から左に
        for j in (1..=c).rev() {
            dp[i][j] = *([dp[i][j], dp[i + 1][j] + 1, dp[i][j + 1] + 1]) // 下と右と比較
                .iter()
                .min()
                .unwrap();
        }
