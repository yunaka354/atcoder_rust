...
fn main() {
    ...
    let mut h: Vec<isize> = vec![0; 100005];
    ...
    dp[1]=0;
    for now in 1..=n {
        chmin!(dp[now+1], dp[now] + (h[now] - h[now+1]).abs());
        chmin!(dp[now+2], dp[now] + (h[now] - h[now+2]).abs());
    }
    let mut place = n;
    ...
    loop {
        ...
        if dp[place-1] == dp[place] - (h[place] - h[place-1]).abs() {
            place -= 1;
        } else {
            place -= 2;
        }
