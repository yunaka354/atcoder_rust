...
fn main() {
    ...
    for _ in 0..n {
        ...
        let entry = g.entry(a).or_insert(Vec::new());
        entry.push(b);
        let entry = g.entry(b).or_insert(Vec::new());
        entry.push(a);
    ...
    que.push_back(1);
    while let Some(next) = que.pop_front() {
        // 既に捜索済み
        ...
        // 到達
        let entry = visited.entry(next).or_insert(false);
        *entry = true;

        // 次の地点をキューに入れる
        for next_floor in g.get(&next).unwrap() {
            que.push_back(*next_floor);
        }
    }
