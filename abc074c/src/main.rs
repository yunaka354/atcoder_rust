...
fn main() {
    ...
    let mut dp = vec![vec![false; f + 1]; f + 1];
    dp[0][0] = true;
    ...
    for water in 0..=f {
        for suger in 0..=f {
            ...
            // 操作1
            if water + suger + 100 * a <= f {
                dp[water + 100 * a][suger] = true;
            }
            // 操作2
            if water + suger + 100 * b <= f {
                dp[water + 100 * b][suger] = true;
            }
            // 操作3
            if water + suger + c <= f {
                dp[water][suger + c] = true;
            }
            // 操作4
            if water + suger + d <= f {
                dp[water][suger + d] = true;
            }
        }
    }
    let mut max_p = 0.0;
    ...
    for water in 1..=f {
        for suger in 1..=f {
            let limit = water * e / 100;
            if dp[water][suger] {
                if suger > limit {
                    continue;
                }
                let p = 100.0 * suger as f64 / (water + suger) as f64;
                if p > max_p {
                    max_p = p;
                    ans = (water, suger);
                }
