...
fn main() {
    ...
    for i in 0..n {
        ...
        for c in 'a'..='z' {
            ...
            // 探しているアルファベットと一致するものが存在する場合
            if let Some(j) = u[i..].iter().position(|&x| x == c) {
                // 現在の位置とスワップする
                u.swap(i, i + j);

                // その先の文字も検索していってスワップし続ける
                for i in i + 1..n {
                    if let Some(j) = u[i..].iter().position(|&x| x == s[i]) {
                        u.swap(i, i + j);
                    }
                }
                // 異なっている数をカウントして、K以内に収まっていればtをアップデートして次のiに行く。
                if (0..n).filter(|&i| s[i] != u[i]).count() <= k {
                    t = u;
                    break;
                }
            }
