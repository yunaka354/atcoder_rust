...
//     n: usize,
//     data: Vec<isize>,
//     lazy: Vec<Option<isize>>, // Noneは遅延がないことを示す
// }
...
//     /// 範囲更新: 指定の範囲をvalueに設定
//     /// [left, right]の閉区間であることに注意。
//     fn update_range(&mut self, left: usize, right: usize, value: isize) {
...
//     // 範囲クエリ: 指定の範囲の最大値を取得
//     // [left, right]の閉区間であることに注意。
//     fn query_range(&mut self, left: usize, right: usize) -> isize {
...
fn main() {
    ...
    let mut tree = LazySegmentTree::new(a);

    for i in 0..m {
        let x = tree.query(b[i], b[i] + 1);
        tree.update(b[i], b[i] + 1, x * -1);
        tree.update(0, n, x / n as isize);
        let re = (x % n as isize) as usize;
        if b[i] + re >= n {
            tree.update(b[i] + 1, n, 1);
            tree.update(0, (b[i] + re) % n + 1, 1);
        } else {
            tree.update(b[i] + 1, (b[i] + re) % n + 1, 1);
        }
    }

    println!("{}", (0..n).map(|i| tree.query(i, i + 1)).join(" "));
}
