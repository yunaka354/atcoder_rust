...
fn main() {
    ...
    let f = |i: &(usize, usize, usize), j: &(usize, usize, usize)| {
        let (i_a, i_b, i_idx) = i; 
        let (j_a, j_b, j_idx) = j;
        let a_num = i_a * (j_a + j_b);
        let b_num = j_a * (i_a + i_b);
        b_num.cmp(&a_num).then_with(|| i_idx.cmp(&j_idx))
    };
    v.sort_by(f);

    println!("{}", v.iter().map(|x| x.2+1).join(" "))
}