...
fn main() {
    ...
    let top_score = v[0].0;
    let top_taste = v[0].1;
    let second_score = v[1].0;
    let second_taste = v[1].1;

    if top_taste != second_taste {
        println!("{}", top_score+second_score);
        return;
    }

    let mut ans = top_score + second_score / 2;
    for (score, taste) in v.iter().skip(2) {
        if top_taste != *taste {
            chmax!(ans, top_score+score);
            break;
        }
    }
    println!("{}", ans);
