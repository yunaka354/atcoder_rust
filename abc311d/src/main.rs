...
fn main() {
    ...
    while let Some((y, x)) = que.pop_front() {
        ...
        for (dy, dx) in DIRECTION_4 {
            ...
            while s[(cy + dy) as usize][(cx + dx) as usize] != '#' {
                cx += dx;
                cy += dy;
                passed[cy as usize][cx as usize] = true;
            }

            v.push((cy, cx));
        }

        for (end_y, end_x) in v {
            if !stopped[end_y as usize][end_x as usize] {
                que.push_back((end_y, end_x));
                stopped[end_y as usize][end_x as usize] = true;
            }
        }
    }
