...
fn main() {
    ...
    let mut cumlative_a = vec![0; n+1];
    let mut cumlative_b = vec![0; m+1];

    for i in 0..n {
        cumlative_a[i+1] = cumlative_a[i] + a[i];
    } 

    for i in 0..m {
        cumlative_b[i+1] = cumlative_b[i] + b[i];
    }

    let mut ans = 0;
    for i in 0..=n {
        if cumlative_a[i] > k { break; }
        let remaining = k - cumlative_a[i];
        let index = match cumlative_b.binary_search(&remaining) {
            Ok(num) => num,
            Err(num) => num - 1,
        };
        chmax!(ans, i+index);
