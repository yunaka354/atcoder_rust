...
fn set_available(
    a: &mut Vec<VecDeque<usize>>,
    available: &mut Vec<usize>,
    que: &mut Vec<(usize, usize)>,
    ai: usize,
) {
    match a[ai].front() {
        None => (),
        Some(x) => {
            if available[*x] != usize::MAX {
                que.push((available[*x], ai))
            } else {
                available[*x] = ai;
            }
        }
...
fn main() {
    ...
    let mut available = vec![usize::MAX; n];
    let mut que = Vec::new();
    ...
    for i in 0..m {
        set_available(&mut a, &mut available, &mut que, i);
    }

    while let Some((i1, i2)) = que.pop() {
        a[i1].pop_front();
        set_available(&mut a, &mut available, &mut que, i1);
        a[i2].pop_front();
        set_available(&mut a, &mut available, &mut que, i2);
    }
