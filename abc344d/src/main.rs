...
fn main() {
    ...
    let t_len = t.len();
    let mut dp = vec![vec![usize::MAX; 105]; 105];
    dp[0][0] = 0;
    for i in 0..n {
        input! { a: usize }
        for tmp in 0..105 {
        ...
        for _ in 0..a {
            ...
            for j in 0..=t_len {
                if dp[i][j] == usize::MAX { continue }; // ここまでに文字列が作れていなければスキップ
                if j+s_len > t_len { continue }; // 連結してTの長さを超えてしまう場合はスキップ
                ...
                if ok {
                    chmin!(dp[i+1][j+s_len], dp[i][j]+1);
                }
            }
        }
    }
    let mut ans = -1;
    if dp[n][t_len] != usize::MAX {
        ans = dp[n][t_len] as isize;
    }
    println!("{}", ans);
}