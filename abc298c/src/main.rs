...
use std::collections::{binary_heap, BTreeSet, BinaryHeap, HashMap, HashSet, VecDeque};
use std::hash::Hash;
...
fn main() {
    ...
    for _ in 0..q {
        ...
        if query_type == 1 {
            input! { number: usize, box_number: usize }
            v[box_number].push(number);
            let entry = map.entry(number).or_insert(BTreeSet::new());
            entry.insert(box_number);

        } else if query_type == 2 {
            input! { box_number: usize }
            v[box_number].sort();
            println!("{}", v[box_number].iter().join(" "));
        } else {
            input! { box_number: usize }
            let cards = map.get(&box_number).unwrap();
