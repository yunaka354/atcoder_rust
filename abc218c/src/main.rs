...
fn rotate_90_degrees_clockwise<T: Copy>(matrix: Vec<Vec<T>>) -> Vec<Vec<T>> {
    let n = matrix.len(); // 行数
    let m = matrix[0].len(); // 列数
    let mut result: Vec<Vec<T>> = vec![vec![matrix[0][0]; n]; m]; // 新しいベクターを初期化
...
fn cut(s: Vec<Vec<char>>, n: usize) -> Vec<Vec<char>> {
    let mut s_top = usize::MAX;
    let mut s_bottom = 0;
    let mut s_left = usize::MAX;
    let mut s_right = 0;
...
fn main() {
    ...
    let s1 = rotate_90_degrees_clockwise(s_cut);
    if s1 == t_cut {
        println!("Yes");
        return;
    }

    let s2 = rotate_90_degrees_clockwise(s1);
    if s2 == t_cut {
        println!("Yes");
        return;
    }

    let s3 = rotate_90_degrees_clockwise(s2);
    if s3 == t_cut {
        println!("Yes");
        return;
    }
