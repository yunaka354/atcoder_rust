...
fn paint_row(remaining_steps: usize, mut d: Vec<Vec<char>>, w: usize, h: usize) -> usize {
    let mut columns = vec![(0, 0); 0];
    for j in 0..w {
        let mut count = 0;
        for i in 0..h {
            if d[i][j] == '.' { count += 1}
        }
        columns.push((count, j));
    }
    columns.sort();
    columns.reverse();
...
fn main() {
    ...
    for t in 0..(1 << h) {
        ...
        for i in 0..h {
            ...
            remaining_steps -= 1;
            for e in d[i].iter_mut() {
                *e = '#';
            }
        }

        if remaining_steps >= 0 {
            let sub_answer = paint_row(remaining_steps as usize, d, w, h);
            chmax!(ans, sub_answer);
        }
    }
