...
fn main() {
    input! {
        n: usize,
        mut reds: [(usize, usize); n],
        mut blues: [(usize, usize); n],
    }

    let mut red_available = vec![true; n];

    blues.sort_by(|a, b| a.0.cmp(&b.0));
    reds.sort_by(|a, b| b.1.cmp(&a.1));

    let mut ans = 0;

    for i in 0..n {
        for j in 0..n {
            if red_available[j] && blues[i].1 > reds[j].1 && blues[i].0 > reds[j].0 {
                red_available[j] = false;
                ans += 1;
                break;
            }
        }
