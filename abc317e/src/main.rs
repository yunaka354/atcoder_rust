...
fn main() {
    ...
    for i in 0..h {
        for j in 0..w {
            let c = a[i][j];
            if c == '>' || c == 'v' || c == '<' || c == '^' {
                arrow.push((c, i as isize, j as isize));
            }
        }
    }
    ...
    for (c, i, j) in &arrow {
        ...
        cy += dy;
        cx += dx;
        while cy >= 0 && cy < h as isize && cx >= 0 && cx < w as isize {
            if a[cy as usize][cx as usize] != '.' {
                break;
            }
            ng.insert((cy, cx));
            cy += dy;
    ...
    while let Some((cy, cx)) = que.pop_front() {
    ...
    if visited[gy][gx] == usize::MAX {
        println!("-1");
    } else {
        println!("{}", visited[gy][gx]);
    }
}
