...
fn main() {
    ...
    // index -> ball
    let mut index_to_ball = (1..=n).collect_vec();

    // ball -> index
    let mut ball_to_index = HashMap::new();

    for (index, ball) in index_to_ball.iter().enumerate() {
        ball_to_index.insert(*ball, index);
    }

    for _ in 0..q {
        input! { x: usize }
        let x_index = *ball_to_index.get(&x).unwrap();
        let mut move_to = x_index;
        if x_index == n-1 {
            move_to -= 1;
        } else {
            move_to += 1;
        }
        let swapped_ball = index_to_ball[move_to];
        ball_to_index.insert(swapped_ball, x_index);
        ball_to_index.insert(x, move_to);
        index_to_ball.swap(x_index, move_to);
    }

    println!("{}", index_to_ball.iter().join(" "));
}