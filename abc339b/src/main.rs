...
fn main() {
    ...
    let mut c = Vec::new();
    for _ in 0..h {
        let s: Vec<char> = std::iter::repeat('.').take(w as usize).collect();
        c.push(s);
    }
    ...
    for _ in 0..n {
        if c[pos.0 as usize][pos.1 as usize] == '.' {
            c[pos.0 as usize][pos.1 as usize] = '#';
            direction = turn_right(direction.0, direction.1);
        } else {
            c[pos.0 as usize][pos.1 as usize] = '.';
            direction = turn_left(direction.0, direction.1);
        }
        pos = (pos.0+direction.0, pos.1+direction.1);

        if pos.0 < 0 { pos.0 = h-1 };
        if pos.0 >= h { pos.0 = 0 };
        if pos.1 < 0 { pos.1 = w-1 };
        if pos.1 >= w { pos.1 = 0 };
    }

    for i in 0..h {
        println!("{}", c[i as usize].iter().join(""));
    }
}