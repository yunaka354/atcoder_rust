...
fn main() {
    ...
    let compress = |txt: Vec<char>| {
        let mut ret = Vec::new();
        let mut counts = 1;
        let mut prev = txt[0];
        for i in 1..txt.len() {
            if txt[i] != prev {
                ret.push((prev, counts));
                prev = txt[i];
                counts = 1;
            } else {
                counts += 1;
    ...
    let new_s = compress(s);
    let new_t = compress(t);
    // println!("{:?}", new_s);
    // println!("{:?}", new_t);
    if new_s.len() != new_t.len() {
        println!("No");
        return;
    }

    for i in 0..new_s.len() {
        if new_s[i].0 != new_t[i].0 {
            println!("No");
            return;
        }
        if new_s[i].1 > new_t[i].1 {
            println!("No");
            return;
        }
        if new_s[i].1 == 1 && new_t[i].1 > 1 {
            println!("No");
            return;
        }
    }
