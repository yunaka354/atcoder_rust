...
fn main() {
    ...
    let l = s.len();
    let mut perm = s.iter().permutations(s.len());
    let mut ans = 0;
    while let Some(p) = perm.next() {
        ...
        for i in 1..l {
            ...
            let s1: String = g1.iter().map(|&&c| c.to_string()).collect();
            let s2: String = g2.iter().map(|&&c| c.to_string()).collect();
            let n1: usize = s1.parse().unwrap();
            let n2: usize = s2.parse().unwrap();
            chmax!(ans, n1*n2);
        }
    }
    println!("{}", ans);
