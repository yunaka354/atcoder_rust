...
fn dfs(mut v: Vec<usize>, ans: &mut usize, n: usize, m: usize, q: usize, abcd: &Vec<(usize, usize, usize, usize)>) {
    ...
    while v.last().unwrap() <= &m {
        dfs(v.clone(), ans, n, m, q, abcd);
...
fn main() {
    ...
    let mut ans = 0;
    dfs(vec![1; 1], &mut ans, n, m, q, &abcd);
    println!("{}", ans);
}