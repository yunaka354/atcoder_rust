...
fn calc(map: &mut HashMap<usize, usize>, x: usize) -> usize {
    if x < 2 {
        return 0
    }
    ...
    if let Some(memo) = map.get(&x) {
        return *memo
    }

    let floor = calc(map, x/2);
    map.insert(x/2, floor);
    let ceil = calc(map, (x+1)/2);
    map.insert((x+1)/2, ceil);
