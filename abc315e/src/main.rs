...
fn main() {
    ...
    let mut g = vec![vec![]; n];
    let mut degree = vec![0 as isize; n];
    let mut ans = Vec::new();
    ...
    for i in 0..n {
        if degree[i] == 0 {
            que.push_back(i); // 最初のキューを準備。最初から入力次数が０のノードを追加。
        }
    }
    ...
    while let Some(node) = que.pop_front() {
        ans.push(node + 1);
        for to in &g[node] {
            degree[*to] -= 1; // 入力次数を減らす
            if degree[*to] == 0 {
                que.push_back(*to); // 入力次数が0になったらキューに追加
            }
        }
    }
    ans.reverse();
    ...
    while let Some(node) = bfs_que.pop_front() {
        visited[node] = true;

        for to in &g[node] {
            if !visited[*to] {
                bfs_que.push_back(*to);
            }
        }
    }
