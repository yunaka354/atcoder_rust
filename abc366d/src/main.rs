...
fn main() {
    ...
    let mut a = vec![vec![vec![0; 101]; 101]; 101];
    let mut s = vec![vec![vec![0; 102]; 102]; 102];
    ...
    for _ in 0..q {
        ...
        let mut ans = 0 as isize;
        ans += s[rx][ry][rz] as isize;
        ans -= s[lx][ry][rz] as isize;
        ans -= s[rx][ly][rz] as isize;
        ans += s[lx][ly][rz] as isize;
        ans -= s[rx][ry][lz] as isize;
        ans += s[lx][ry][lz] as isize;
        ans += s[rx][ly][lz] as isize;
        ans -= s[lx][ly][lz] as isize;
        println!("{}", ans);
    }
}
