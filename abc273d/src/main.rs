...
#[allow(dead_code)]
fn lower_bound2<T: Ord>(arr: &Vec<T>, x: T) -> usize {
    let mut left = -1;
    let mut right = arr.len() as isize;
...
fn main() {
    ...
    let mut row_walls: HashMap<isize, Vec<isize>> = HashMap::new();
    let mut col_walls: HashMap<isize, Vec<isize>> = HashMap::new();

    for i in 0..n {
        let (r, c) = rc[i];
        let r_entry = row_walls.entry(r).or_insert(vec![0, w + 1]);
        let c_entry = col_walls.entry(c).or_insert(vec![0, h + 1]);
        r_entry.push(c);
        c_entry.push(r);
    }

    for (_k, v) in row_walls.iter_mut() {
        v.sort();
    }

    for (_k, v) in col_walls.iter_mut() {
        v.sort();
    }

    for (d, l) in dl {
        if d == 'L' {
            match row_walls.get(&rs) {
                None => {
                    cs = max(cs - l, 1);
                }
                Some(v) => {
                    let idx = lower_bound2(v, cs);
                    let wall = v[idx];
                    cs = max(cs - l, wall + 1);
                }
            }
            ...
            match row_walls.get(&rs) {
                ...
                Some(v) => {
                    let idx = lower_bound(v, cs);
                    let wall = v[idx];
                    cs = min(cs + l, wall - 1);
                }
            }
        } else if d == 'D' {
            match col_walls.get(&cs) {
                None => {
                    rs = min(rs + l, h);
                }
                Some(v) => {
                    let idx = lower_bound(v, rs);
                    let wall = v[idx];
                    rs = min(rs + l, wall - 1);
                }
            }
