...
fn main() {
    ...
    let func = |span: usize, speed: usize, rest: usize | {
        let chunk = x / (span + rest);
        let residual = x % (span + rest);

        let mut dist = span * speed * chunk;
        dist += speed * min!(residual, span);
        dist
    };

    let takahashi = func(a, b, c);
    let aoki = func(d, e, f);
