...
fn main() {
    ...
    for c in s {
        current_pos = match c {
            'R' => (current_pos.0+1, current_pos.1),
            'L' => (current_pos.0-1, current_pos.1),
            'U' => (current_pos.0, current_pos.1+1),
            'D' => (current_pos.0, current_pos.1-1),
            _ => panic!("error")
        };
        if set.contains(&current_pos) {
            println!("Yes");
            return;
        }
        set.insert(current_pos);
