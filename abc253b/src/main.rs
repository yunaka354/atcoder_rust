...
fn main() {
    ...
    while let Some((y, x)) = que.pop_front() {
        for (dy, dx) in DIRECTION_4 {
            ...
            if ny < 0 || ny >= h as isize || nx < 0 || nx >= w as isize || visited[ny as usize][nx as usize] {
                continue;
            }

            visited[ny as usize][nx as usize] = true;

            cost[ny as usize][nx as usize] = cost[y as usize][x as usize] + 1;
    ...
    println!("{}", cost[g.0 as usize][g.1 as usize]);
}