...
fn main() {
    ...
    for _ in 0..n {
        input! { r: Usize1, c: Usize1, e: isize}
        meds.insert((r, c), e);
    }

    let mut energies = vec![vec![isize::MIN; w]; h];
    ...
    while let Some((cy, cx, current_energy)) = que.pop_front() {
        let updated_energy = match meds.remove(&(cy, cx)) {
            Some(e) => {
                max(current_energy, e) - 1
            },
            None => current_energy - 1
        };

        // 移動後のエネルギーが0未満だと移動できない
        if updated_energy < 0 {
            continue;
        }
        ...
        for (dy, dx) in DIRECTION_4 {
            ...
            if a[ny][nx] == '#' {continue;}
            if energies[ny][nx] >= updated_energy {
                continue;
            }

            energies[ny][nx] = updated_energy;
            que.push_back((ny, nx, updated_energy));
        }
    }
