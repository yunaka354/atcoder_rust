...
fn main() {
    ...
    let m = 2147483647;
    let mut t: Vec<isize> = vec![0; 200005];
    let mut h: Vec<isize> = vec![0; 200005];
    let mut power100: Vec<isize> = vec![0; 200005];
    for i in 1..=n {
        t[i] = (s[i-1] as u64 - 97) as isize;
    }

    power100[0] = 1;

    for i in 1..=n {
        power100[i] = power100[i-1] * 100 % m;
    }
    ...
    let hash_value = |l: usize, r: usize| {
        let mut val = h[r] - (h[l-1] * power100[r - l + 1] % m);
        if val < 0 {val += m};
        val
    };
    ...
    for _ in 0..q {
        ...
        let hash1 = hash_value(a, b);
        let hash2 = hash_value(c, d);
        if hash1 == hash2 {
            println!("Yes");
        } else {
            println!("No");
