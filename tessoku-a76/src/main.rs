...
fn main() {
    ...
    for i in 1..=n+1 {
        let pos_l = if x[i] > r {x.binary_search(&(x[i]-r)).unwrap_or_else(|x|x)} else {0};
        let pos_r = if x[i] > l {x.binary_search(&(x[i]-l+1)).unwrap_or_else(|x|x)-1} else {0};

        dp[i] = if x[i]+1 > l {sum[pos_r]} else {0};
        if pos_l >= 1 {
            dp[i] -= sum[pos_l-1];
        }
        dp[i] = (dp[i]+ MOD) % MOD;
