...
fn lower_bound(v: &Vec<usize>, searched_value: usize) -> Result<usize, usize> {
    v.binary_search_by(|element| match element.cmp(&searched_value) {
        Ordering::Equal => Ordering::Greater,
        ord => ord,
    })
    // Check what we found.
    .or_else(|idx| {
        if v.get(idx) == Some(&searched_value) {
            Ok(idx)
        } else {
            Err(idx)
        }
...
fn upper_bound(v: &Vec<usize>, searched_value: usize) -> Result<usize, usize> {
    v.binary_search_by(|element| match element.cmp(&searched_value) {
...
fn main() {
    ...
    let mut ans = 0;
    for i in 0..n {
        let a_count = match lower_bound(&a, b[i]) {
            Ok(idx) => idx,
            Err(idx) => idx,
        };

        let c_count = match upper_bound(&c, b[i]) {
            Ok(idx) => n - idx,
            Err(idx) => n - idx,
        };
