...
impl SegmentTree {
    ...
    fn update(&mut self, mut pos: usize, x: isize) {
        ...
        while pos > 0 {
            pos = (pos - 1) / 2;
            self.dat[pos] = max(self.dat[pos * 2 + 1], self.dat[pos * 2 + 2]);
        }
    }
    ...
    fn query(&self, l: isize, r: isize, a: isize, b: isize, u: usize) -> isize {
        // 一切含まれない場合。
        if r <= a || b <= l {
            return std::isize::MIN; // 問題によってこの部分は変更すること。
        }
        // 完全に含まれる場合
        if l <= a && b <= r {
            return self.dat[u];
...
fn main() {
    ...
    for _ in 0..q {
        ...
        match query {
            1 => {
                input! {pos: Usize1, x: usize};
                tree.update(pos, x as isize);
            }
            2 => {
                input! {l: Usize1, r: Usize1};
                println!(
                    "{}",
                    tree.query(l as isize, r as isize, 0, tree.size as isize, 0)
                );
            }
            _ => panic!("error"),
        }
