...
fn main() {
    ...
    let mut painted_row = HashSet::new();
    let mut painted_col = HashSet::new();

    let mut sum = 0;
    for (t, a, x) in tax {
        let entry = map.entry(x).or_insert(0);
        if t == 1 {
            if !painted_row.contains(&a) {
                let add = w - painted_col.len();
                *entry += add;
                sum += add;
                painted_row.insert(a);
            }
        } else {
            if !painted_col.contains(&a) {
                let add = h - painted_row.len();
                *entry += h - painted_row.len();
                sum += add;
                painted_col.insert(a);
            }
        }
    }

    let entry = map.entry(0).or_insert(0);
    *entry += h * w - sum;
