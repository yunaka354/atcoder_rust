...
fn main() {
    ...
    for _ in 0..q {
        ...
        match query {
            1 => {
                ...
                let mut pre_node = g.get(&x).unwrap().clone();
                let new_node = Node {
                    pre: Some(x),
                    post: pre_node.post,
                };
                if let Some(post_of_pre) = pre_node.post {
                    let mut post_node = g.get(&post_of_pre).unwrap().clone();
                    post_node.pre = Some(y);
                    g.insert(post_of_pre, post_node);
                }
                pre_node.post = Some(y);
            ...
            2 => {
                input! {x: usize};
                let to_be_deleted = g.get(&x).unwrap().clone();

                if let Some(idx) = to_be_deleted.pre {
                    let mut pre_node = g.get(&idx).unwrap().clone();
                    pre_node.post = to_be_deleted.post;
                    g.insert(idx, pre_node);
                }

                if let Some(idx) = to_be_deleted.post {
                    let mut post_node = g.get(&idx).unwrap().clone();
                    post_node.pre = to_be_deleted.pre;
                    g.insert(idx, post_node);
                }
    ...
    let mut ans = Vec::new();
    while let Some(next) = g.get(&start) {
        ans.push(start);
        match next.post {
            Some(to) => {
                start = to;
