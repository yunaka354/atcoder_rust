...
fn main() {
    ...
    let mut l_map: HashMap<usize, usize> = HashMap::new();
    ...
    for i in 0..n {
        ...
        match direction {
            'R' => {
                let result = l_map.get(&y);
                if let Some(l_max) = result {
                    if x < *l_max {
                        println!("Yes");
                        return;
                    }
                }
                let entry = r_map.entry(y).or_insert(usize::MAX);
                *entry = min(*entry, x);
            },
            'L' => {
                let result = r_map.get(&y);
                if let Some(r_min) = result {
                    if x > *r_min {
                        println!("Yes");
                        return;
                    }
                }
                let entry = l_map.entry(y).or_insert(0);
                *entry = max(*entry, x);
            },
            _ => panic!("error")
        };
