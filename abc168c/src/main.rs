...
fn main() {
    ...
    let mut a_angle: f64 = minutes / 2.0;
    let mut b_angle: f64 = m * 6.0;
    if a_angle < b_angle {
        swap(&mut a_angle, &mut b_angle);
    }
    let angle_radians = (a_angle - b_angle) * (std::f64::consts::PI / 180.0);
    let x = a.powi(2) + b.powi(2) - 2.0 * a * b * angle_radians.cos();
    println!("{}", x.sqrt());
}