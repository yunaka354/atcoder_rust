...
fn main() {
    input! {
        ...
        cloth_count: usize,
        temps: [usize; days],
        clothes: [(usize, usize, isize); cloth_count],
    }

    // dp[day][cloth_type]
    let mut dp = vec![vec![-1; cloth_count]; days];

    // first day
    for i in 0..cloth_count {
        let (lower_temp, higher_temp, _value) = clothes[i];
        if lower_temp <= temps[0] && higher_temp >= temps[0] {
            dp[0][i] = 0;
        }
    }
    ...
    for day in 1..days {
        for prev_cloth in 0..cloth_count {
            for now_cloth in 0..cloth_count {
                if dp[day-1][prev_cloth] == -1 {
                    continue;
                }
                let (_prev_lower_temp, _prev_higher_temp, prev_value) = clothes[prev_cloth];
                let (now_lower_temp, now_higher_temp, now_value) = clothes[now_cloth];
                if now_lower_temp <= temps[day] && now_higher_temp >= temps[day] {
                    chmax!(dp[day][now_cloth], dp[day-1][prev_cloth]+(prev_value - now_value).abs());
                }
            }         
        }
    }
