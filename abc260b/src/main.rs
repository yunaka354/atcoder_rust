...
fn main() {
    ...
    for i in 0..n {
        student.push((i, a[i], b[i], a[i]+b[i]));
    }
    let mut ans = Vec::new();
    student.sort_by(|a, b| b.1.cmp(&a.1).then(a.0.cmp(&b.0)));
    
    for _ in 0..x {
        let tmp = student.remove(0);
        ans.push(tmp);
    }

    student.sort_by(|a, b| b.2.cmp(&a.2).then(a.0.cmp(&b.0)));
    ...
    student.sort_by(|a, b| b.3.cmp(&a.3).then(a.0.cmp(&b.0)));
