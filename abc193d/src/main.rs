...
fn point(s: &Vec<char>) -> usize {
    ...
    for i in 1..=9 {
        ret += i * (10 as usize).pow(v[i] as u32);
    }
    ret
}
...
fn main() {
    ...
    let mut remaining = vec![k as isize; 10];
    ...
    for i in 1..10 {
        for j in 1..10 {
            s[4] = (i as u8 + b'0') as char;
            t[4] = (j as u8 + b'0') as char;
            if point(&s) > point(&t) {
                if i != j {
                    s_win += remaining[i] * remaining[j];
                } else {
                    s_win += remaining[i] * (remaining[j] - 1);
                }
            }
        }
    }

    println!("{}", s_win as f64 / ((9 * k - 8) * (9 * k - 9)) as f64);
}
