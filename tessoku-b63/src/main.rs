...
fn main() {
    ...
    let mut distance = vec![vec![-1; c+1]; r+1];
    
    que.push_back((sy-1, sx-1));
    distance[sy-1][sx-1] = 0;

    while !que.is_empty() {
        let (cy, cx) = que.pop_front().unwrap();
        for (dy, dx) in DIRECTION_4 {
            let ny = cy as isize + dy; let nx = cx as isize + dx;
            let ny = ny as usize; let nx = nx as usize;            
            if maze[ny][nx] != '#' && distance[ny][nx] == -1 {
