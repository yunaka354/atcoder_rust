...
fn main() {
    ...
    let mut at_count_s = 0;
    let mut at_count_t = 0;
    for c in &s {
        if *c == '@' {
            at_count_s += 1
        }
    ...
    for c in 'a'..='z' {
        let count_s = s.iter().filter(|e| *e==&c).count();
        let count_t = t.iter().filter(|e| *e==&c).count();
        let count_s = count_s as isize;
        let count_t = count_t as isize;

        if count_s != count_t && !['a', 't', 'c', 'o', 'd', 'e', 'r'].contains(&c) {
            println!("No");
            return;
        }
        
        if count_s > count_t {
            at_count_t -= count_s - count_t;
        } else {
            at_count_s -= count_t - count_s;
        }
    }

    if at_count_s < 0 || at_count_t < 0 {
        println!("No");
    } else {
        println!("Yes");
    }
