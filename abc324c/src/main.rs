...
fn check(t: &Vec<char>, s: &Vec<char>) -> usize {
    for i in 0..t.len() {
        if i >= s.len() {
            return i;
        }
...
fn main() {
    ...
    for i in 0..n {
        let mut flag = false;
        if a[i] == t.len() && t.len() == s[i].len() { flag = true }
        if a[i]+b[i] >= t.len() - 1 && t.len()-1 == s[i].len() { flag = true}
        if a[i]+b[i] >= t.len() && s[i].len()-1 == t.len() { flag = true }
        if a[i]+b[i] == t.len() - 1 && t.len() == s[i].len() { flag = true }
